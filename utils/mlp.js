...
export class MLP {
  constructor(...args) {
    ...
    this.epochs = 1200;
    this.activation_input = sigmoid;
    this.activation_hidden = relu;
    this.activation_output = linear;
    this.lr = args[3];
    this.l2 = args[4];
    this.dropout_prob = 0.00001;
    this.lambda = 0.00001;
  ...
  train(input, target) {
    ...
    for (let i = 0; i <= this.epochs; i++) {
      ...
      let hidden_layer_activated = hidden_layer_logits.map(v => this.activation_hidden(v, false));
      ...
      let output_layer_activated = output_layer_logits.map(v => this.activation_output(v, false));

      let output_error = subtract(target, output_layer_activated);
      let output_delta = dotMultiply(output_error, output_layer_logits.map(v => this.activation_output(v, true)));
      let hidden_error = multiply(output_delta, transpose(this.synapse_one));
      let hidden_delta = dotMultiply(hidden_error, hidden_layer_logits.map(v => this.activation_hidden(v, true)));
  ...
  predict(input) {
    let input_layer = input;
    let hidden_layer = multiply(input_layer, this.synapse_zero).map(v => this.activation_hidden(v, false));
    let output_layer = multiply(hidden_layer, this.synapse_one).map(v => this.activation_output(v, false));
    return output_layer;
  }
}
